...
def set_kb_ids(case_ids: list = None, standard_ids: list = None):
    """
    动态设置知识库 ID, 在基础库的基础上追加企业特定知识库

    Args:
        case_ids: 案例库 ID 列表
        standard_ids: 规范库 ID 列表
    """
    global CASE_KB_IDS, STAND_KB_IDS
    if case_ids:
        CASE_KB_IDS.append(case_ids)
    if standard_ids:
        STAND_KB_IDS.append(standard_ids)
